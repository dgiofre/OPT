...
void PairRuNNer::settings(int narg, char **arg) {
    ...
    if(comm->me == 0) {
        if(screen) {
            ...
            fprintf(screen, "Maximum allowed extrapolation warnings: %li\n", maxew);
        }
        if(logfile) {
            ...
            fprintf(logfile, "Maximum allowed extrapolation warnings: %li\n", maxew);
        }
    }
...
double PairRuNNer::calc_G_dG(int ca, RuNNer_forces *F, RuNNer_symfunc *G, RuNNer_neighbor *N) {
    ...
    if(value < min || value > max) {
        if(showew == 1) {
            if(screen) {
                fprintf(screen, "PROC %d: ### EXTRAPOLATION WARNING ### TS: %ld ATOM: %d SYMFUNC: %d TYPE: %d VALUE: %f MIN: %f MAX: %f COUNT: %li \n",
                    comm->me, update->ntimestep, ca, num, type, value, min, max, ewcount);
            }
            if(logfile) {
                fprintf(logfile, "PROC %d: ### EXTRAPOLATION WARNING ### TS: %ld ATOM: %d SYMFUNC: %d TYPE: %d VALUE: %f MIN: %f COUNT: %li \n",
                    comm->me, update->ntimestep, ca, num, type, value, min, max, ewcount);
            }
        }
//#ifdef _OPENMP
        ...
        for(int j=0; j<Nnum; j++) {
            ...
            if(rij<=rc && e1 == Nej) {  
                ...
                for(int isym=0; isym<numSF; isym++) {
                    ...
                    yptr[j] -= p1 * dyij;
                    zptr[j] -= p1 * dzij;
                    //fprintf(logfile, "Type2 distance xij %f , yij %f , zij %f \n", xptr[j], yptr[j], zptr[j]);
                }
            }
        }
    }
                            ...
                            if(r2jk <= rc2) {
                                ...
                                for(int isym=0; isym<numSF; ++isym) {
                                    ...
                                    */
                                    ...
                                    //fprintf(logfile, "Type3 p1  %f , p2 %f , p3 %f\n", p1, p2, p3);
    ...
    for(int isym=0; isym<numSF; isym++) {
        ...
        if(value[isym] < min[isym] || value[isym] > max[isym]) {
            if(showew == 1) {
                if(screen) {
                    fprintf(screen, "PROC %d: ### EXTRAPOLATION WARNING ### TS: %ld ATOM: %d SYMFUNC: %d TYPE: %d VALUE: %f MIN: %f MAX: %f COUNT: %li \n",
                        comm->me, update->ntimestep, ca, index[isym], type, value[isym], min[isym], max[isym], ewcount);
                }
                if(logfile) {
